...
class _Store:
    def __init__(self, root: Path, **data: object) -> None:
        self.store_path = root / "tool_data" / "bash.json"
        self.data = data
        self.reloaded = False
...
@pytest.mark.asyncio
async def test_bash_tool_runs_standard_shell_syntax_anywhere_in_container(
...
@pytest.mark.asyncio
async def test_bash_tool_rejects_invalid_command_and_missing_cwd(monkeypatch, tmp_path: Path):
    calls = _fake_bash(monkeypatch)
    store = _Store(tmp_path)
...
@pytest.mark.asyncio
async def test_bash_tool_allows_commands_not_present_in_old_whitelist(monkeypatch, tmp_path: Path):
    calls = _fake_bash(monkeypatch)
    store = _Store(tmp_path)
...
@pytest.mark.asyncio
async def test_bash_tool_injects_a_persistent_persona_runtime_environment(
    monkeypatch, tmp_path: Path
):
    calls = _fake_bash(monkeypatch)
...
@pytest.mark.asyncio
async def test_bash_tool_makes_the_restricted_docker_cli_available(monkeypatch, tmp_path: Path):
    calls = _fake_bash(monkeypatch)
...
@pytest.mark.asyncio
async def test_bash_tool_routes_docker_compose_through_the_same_proxy(monkeypatch, tmp_path: Path):
    calls = _fake_bash(monkeypatch)
...
@pytest.mark.asyncio
async def test_bash_tool_clamps_output_to_persona_policy(monkeypatch, tmp_path: Path):
...
async def test_bash_docker_inspect_sends_a_status_card_and_hides_internal_marker(
    ...
    class _Executor:
        async def execute_async(self, tool, params, invocation_context=None):
            sent["params"] = params
