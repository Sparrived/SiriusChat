...
│   ├── diary/                  ← 日记条目与索引（v1.0）
│   ├── glossary/               ← 名词解释（v1.0）
│   └── user_memory/            ← 用户档案（v1.0，简化 UserProfile）
├── token/                      ← Token 用量统计
...
legacy `run_live_message` 队列系统已在 v1.0 中移除。请直接使用 `EmotionalGroupChatEngine.process_message()`。

### 5. 引擎管理

**EmotionalGroupChatEngine（v1.0 默认）**：
