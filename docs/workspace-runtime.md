...
├── memory/                     ← 记忆系统数据
│   ├── user_memory/            ← 用户事实记忆
│   ├── event_memory/           ← 事件记忆（legacy）
│   ├── working_memory.json     ← 工作记忆（v0.28）
│   ├── episodic/               ← 情景记忆（v0.28）
│   └── semantic/               ← 语义记忆（v0.28）
├── token/                      ← Token 用量统计
...
1. 创建所有目录（如果不存在）
2. 加载 SKILL（从 `skills/` 目录 + 内置 SKILL）
...
`WorkspaceConfig` 包含：
- `active_agent_key`：当前选中的角色
- `orchestration`：编排策略（冷却时间、批处理阈值、分割标记等）
- `providers`：模型提供者列表
...
如果配置变化涉及 provider 或 agent，引擎会被重置（legacy 引擎下次使用时重新创建）。
...
每个 `session_id` 有独立的：
- **消息队列**（`deque`）：非阻塞入队
- **Drain 任务**（`asyncio.Task`）：顺序处理队列中的消息，受 `asyncio.Lock` 保护
- **Transcript 缓存**：内存中的聊天记录，定期写入 `SessionStore`
- **Participants 元数据**：用户 ID、名称、角色等

**批处理**：当队列中积压的消息超过 `pending_message_threshold` 时，runtime 会尝试合并来自同一说话人的连续消息，减少引擎调用次数。

**冷却**：如果 `min_reply_interval_seconds` 设置了，runtime 会在冷却期内积累消息，冷却结束后一次性处理。

### 5. 引擎管理

WorkspaceRuntime 同时支持两种引擎：

**Legacy 引擎（AsyncRolePlayEngine）**：
- 懒加载：第一次 `_get_engine()` 时才创建
- 缓存：创建后保存在 `self._engine`
- 配置变更时自动重置

**v0.28 引擎（EmotionalGroupChatEngine）**：
- 通过 `create_emotional_engine()` 显式创建
- **不缓存**：每次调用返回新实例，由调用方管理生命周期
- 自动加载 persona（从 `active_agent_key` 对应的 roleplay preset）
- 自动注入 SKILL 运行时
...
[重新加载 workspace.json]
    │
    ▼ 涉及 provider/agent
[_reset_engine_state] 停止后台任务，丢弃 legacy 引擎缓存
    │
    ▼
[下次消息处理时] 用新配置创建引擎
```
