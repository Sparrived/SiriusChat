...
从 `v0.23.0` 开始，推荐由 workspace 层统一接管持久化：

- 外部只提供 `work_path` 和业务输入
- `WorkspaceLayout` 统一声明所有文件与目录路径
- `WorkspaceRuntime` 统一管理 session 恢复、自动落盘、参与者元数据和多 session 锁
- `WorkspaceMigrationManager` 负责从旧根目录平铺布局迁移到新 workspace 布局
...
- `sirius_chat/providers/routing.py`
  - provider key 注册表（`providers/provider_keys.json`）、支持平台清单、自动路由 provider（优先按 `ProviderConfig.models` 显式模型列表匹配，其次按 `healthcheck_model` 精确匹配，最后使用启发式前缀规则）与框架级 Provider 检测流程。
  - 新增 `WorkspaceProviderManager`，把 provider 注册表管理收敛到 workspace 级接口。
- `sirius_chat/workspace/layout.py`
  - `WorkspaceLayout` 是所有持久化路径的单一事实来源，统一生成 `workspace.json`、`config/`、`providers/`、`sessions/`、`memory/`、`token/`、`roleplay/`、`skills/`、`skill_data/` 等路径。
- `sirius_chat/workspace/runtime.py`
  - `WorkspaceRuntime` 是推荐的高层入口：自动初始化 workspace、检测迁移、恢复 transcript、调用 engine 并在成功后保存 session store 与 participants 元数据。
- `sirius_chat/workspace/migration.py`
  - `WorkspaceMigrationManager` 负责识别旧的根目录平铺布局，并把 session/provider/memory/token/roleplay 文件迁移到新 workspace 结构。
...
1. 外部通过 `WorkspaceRuntime.open(work_path)` 或低层 `SessionConfig` 入口启动会话。
