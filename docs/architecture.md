...
- `sirius_chat/providers/newapi.py`
  - NewAPI 专用适配（兼容 OpenAI API 格式），固定使用内部基地址 `https://docs.newapi.pro`。
...
- `sirius_chat/core/intent_v2.py` ✨ **(意图分析 v2)**
  ...
  - LLM 路径：增强上下文（参与者列表 + 近期消息），解析意图类型、target、重要性。
  - 关键词回退路径：基于模式匹配 + 参与者名称匹配，零 LLM 开销。
  - 通过 `OrchestrationPolicy.enable_intent_analysis` 控制是否启用 LLM 路径（默认启用）。
...
2. **意图分析 v2**（`IntentAnalyzer`）：
   - LLM 路径（`enable_intent_analysis=True`）：发送含参与者列表和近 8 条消息上下文的结构化提示，解析 `target`、`intent_type`、`importance`
   - 关键词回退路径（默认或 LLM 失败时）：基于问号、请求关键词、短反应语匹配，额外检查参与者名字判定 target
...
- 任务级编排当前包含 `memory_extract`、`event_extract` 与 `memory_manager`，并使用 token 预算控制。
