...
补充说明：`ConfigManager` 会在构建 `SessionConfig` 前把旧版 `enable_intent_analysis` / `intent_analysis_model` 自动规范化到 `task_enabled["intent_analysis"]` / `task_models["intent_analysis"]`，并把旧版 `message_debounce_seconds` 按四舍五入映射到 `pending_message_threshold`；新的模板与持久化快照不再写回这些旧字段。
...
3. processor 在 session 锁内构建 `SessionConfig`，依据 `pending_message_threshold` 决定逐条处理还是进入静默批处理。
