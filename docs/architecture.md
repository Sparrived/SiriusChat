...
1. **感知层**：注册参与者到群隔离的 `user_memory`；写入 `working_memory` 滑动窗口（含助手回复，按 FIFO + protected 截断）；将 human 消息缓冲到 `event_memory.buffer_message(user_id, content, group_id)`（过短内容自动丢弃）；更新 `group_last_message_at`。
