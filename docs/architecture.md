...
- `sirius_chat/session/store.py` ✨ **(包重构)**
  ...
  - 默认 Store：`SqliteSessionStore`（`session_state.db`）；如需 JSON 格式可显式传入 `JsonSessionStore`。
...
- 通过 `SqliteSessionStore`（默认）或 `JsonSessionStore` 可在重启后恢复 transcript、participant_memories 与摘要。迁移说明见 `docs/migration-v0.19.md`。
...
**TD-12 Provider 类型通过 `getattr` 运行时检测**
- `_call_provider` 用 `getattr` + duck typing 绕过了已定义的 `AsyncLLMProvider` / `LLMProvider` Protocol，静态分析无法发现不合规 provider。
- 修复方案：为两个 Protocol 添加 `@runtime_checkable`，改用 `isinstance` 判断。

**TD-13 `async_engine/` 与 `core/` 关系倒置**
- `copilot-instructions.md` 描述"编排逻辑集中在 `sirius_chat/async_engine.py`"，但实际逻辑在 `sirius_chat/core/engine.py`；`async_engine/` 只是再导出包。文档描述与代码结构不符。

### P3 — 低优先级（长期债务）

**TD-09 `AsyncRolePlayEngine` 是神类（2576 行，54 方法）**
- 承担了 LLM 调用、会话生命周期、记忆提取、技能执行、参与度决策、后台任务调度、令牌追踪等全部职责。
- 长期目标：按职责拆分为 `SessionLifecycleManager`、`MemoryTaskRunner`、`EngagementPipeline` 等协作服务。

**TD-11 `LiveSessionContext` 16 字段万能大厅**
- 混合了存储引用、运行时计数器、子系统实例、并发控制、请求中间态等 4 个抽象层次，边界模糊。
