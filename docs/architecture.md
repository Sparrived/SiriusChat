...
会话初始化后，`BackgroundTaskManager` 会按 `OrchestrationPolicy.consolidation_*` 配置静默启动归纳循环，整理事件、摘要与事实，控制记忆膨胀。该循环不再提供单独开关；若需停用相关 LLM 调用，应关闭 `task_enabled["memory_manager"]`。
