...
- `WorkspaceRuntime` 统一管理 session 恢复、自动落盘、参与者元数据、多 session 锁，以及配置文件变更后的下一轮热刷新
...
- `sirius_chat/workspace/runtime.py`
  ...
  - 运行时会在每次 `run_live_message(...)` 前检查 config root 中的 workspace/config/provider/roleplay 文件签名；若检测到外部修改，则重建 engine 上下文并应用新配置，但保留既有 transcript 与 session store。
