...
- `sirius_chat/skills/` ✨ **(SKILL系统)**
  ...
  - `registry.py`：`SkillRegistry`，从 `{work_path}/skills/` 目录自动发现并加载 Python SKILL 文件（需导出 `SKILL_META` 字典和 `run()` 函数）。若目录不存在，会自动创建 `skills/` 与 `README.md` 引导文档；加载前自动调用依赖解析器安装缺失包（受 `auto_install_skill_deps` 控制）。
