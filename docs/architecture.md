...
| `EmotionalGroupChatEngine` | **v0.28+ 默认引擎**：情感化群聊场景，需要情感分析、延迟响应、主动发起、群隔离记忆 | 四层认知架构编排、三层记忆底座、后台任务、事件流 |
...
会话初始化后（legacy 路径），`BackgroundTaskManager` 会按 `OrchestrationPolicy.consolidation_*` 配置静默启动归纳循环。该循环不再提供单独开关。
...
- **Legacy 引擎 `AsyncRolePlayEngine` 已在 v0.28 归档**，代码位于 `sirius_chat/core/_legacy/`。新用户无需关注。存档文档见 `docs/engine-legacy.md`。
...
3. 修改 engine 主流程：优先检查 `core/emotional_engine.py` 及其 helper（`cognition.py`、`response_assembler.py`、`model_router.py`），不要把 provider 细节塞进 core。
