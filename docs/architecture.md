...
- 当前推荐入口是 `WorkspaceRuntime` 或直接创建 `EmotionalGroupChatEngine`。
- **v1.0.0**：`EmotionalGroupChatEngine` 是唯一默认引擎；`AsyncRolePlayEngine` 已完全移除。
...
| `sirius_chat/core/` | 编排核心：`EmotionalGroupChatEngine`、意图分析、情感分析、响应策略、阈值引擎、节奏分析、事件总线、身份解析 | 不负责 workspace 文件发现与目录组织 |
| `sirius_chat/async_engine/` | 兼容导出、提示词/任务配置/工具函数辅助层 | 不是持久化所有者，也不是 engine 真正实现位置 |
| `sirius_chat/memory/` | 基础记忆（工作窗口+热度+归档）、日记记忆（LLM生成+检索）、用户管理（简化UserProfile）、名词解释（Glossary）。**v1.0.0 新增**：ContextAssembler、IdentityResolver | 不直接决定 provider 路由 |
...
- `sirius_chat/core/identity_resolver.py`：跨平台身份解析器。
- `sirius_chat/memory/context_assembler.py`：上下文组装器（基础记忆 + 日记记忆 → OpenAI messages）。
...
1. **感知层**：`IdentityResolver.resolve()` 解析跨平台身份；`UserManager.register()` 注册参与者（群隔离）；`BasicMemoryManager.add_entry()` 写入按群滑动窗口（硬限制 30 条，上下文窗口 5 条）；`HeatCalculator.compute_heat()` 更新群体热度；更新 `group_last_message_at`。
2. **认知层（统一）**：`CognitionAnalyzer` 联合规则引擎分析情绪+意图（零成本热路径，~90% 命中）；LLM fallback 处理复杂情况（~10% 命中）；`DiaryManager.retrieve()` + `BasicMemoryManager.get_context()` 获取相关记忆。
...
4. **执行层**：`ResponseAssembler` 返回 `PromptBundle`（`system_prompt` 包含 persona、情绪、共情策略、日记引用、glossary、skill 描述与输出格式指令；`user_content` 为当前消息的格式化内容）。`ContextAssembler.build_messages()` 将基础记忆最近 n 条 + 日记检索 top_k 条组装为标准 OpenAI `messages` 格式（`user`/`assistant`/`system`）。`StyleAdapter` 动态调整 `max_tokens` / `temperature` / `tone`。`ModelRouter` 按任务感知选择模型，调用 provider 生成回复。
