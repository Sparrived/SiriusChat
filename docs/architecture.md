...
- `sirius_chat/providers/routing.py`
  - provider key 注册表（`provider_keys.json`）、支持平台清单、自动路由 provider（优先按 `healthcheck_model` 与完整模型名精确匹配）与框架级 Provider 检测流程。
...
- `sirius_chat/async_engine/` ✨ **(P0-003 重构)**
  ...
  - `core.py` (500+ 行)：AsyncRolePlayEngine 类（主编排引擎，保持公开 API 不变）
    * 公开方法：`run_session()` 单轮会话、`run_live_session()` 持续动态会话
  ...
  - `prompts.py` (90+ 行)：系统提示构建
    * `build_system_prompt()`：整合 agent 身份、时间、用户记忆、编排指令的完整提示词
    * 支持按类别分组记忆、置信度标记、冲突提示、安全约束
...
- `sirius_chat/cache/` ✨ **(P2-001)**
  ...
  - `redis.py`：`RedisCache` 分布式实现，支持跨进程会话共享（可选 Redis 依赖）。
