...
2. 把消息加入 `WorkingMemoryManager`（该群的滑动窗口；助手回复也会写入，按动态 importance 截断，消息名经过 sanitize）
3. 更新群聊活跃度时间戳
...
5. `parse_dual_output()` 分离 `<think>`（内心独白）和 `<say>`（说出口的话）
6. `<think>` 存入 `AutobiographicalMemoryManager`，形成第一人称日记；同时支持 `add_glossary_term()` 与 `build_glossary_prompt_section()`
7. `_process_skill_calls()` 解析并执行 `[SKILL_CALL: ...]` 标记（内置技能含 `learn_term`、`url_content_reader`、`bing_search`；`silent=True` 时结果不追加到回复文本）
...
# result["thought"] 为模型产生的内心独白（来自 <think> 标签）
```
