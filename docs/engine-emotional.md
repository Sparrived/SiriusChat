...
2. 把消息加入 `WorkingMemoryManager`（该群的滑动窗口）
3. 更新群聊活跃度时间戳
...
**做什么**：并行分析情绪、意图、检索记忆。三个任务是**逻辑上并行**的（虽然当前实现是顺序执行，因为共享了 analyzer 依赖）。

| 分析器 | 输入 | 输出 | 默认模型 |
|--------|------|------|---------|
| **EmotionAnalyzer** | 消息内容 + 上下文 | `EmotionState`（valence/arousal/基本情绪） | gpt-4o-mini |
| **IntentAnalyzerV3** | 消息内容 + 上下文 + 情绪 | `IntentAnalysisV3`（社交意图/紧急度/相关性/是否@我） | gpt-4o-mini |
...
| **记忆 promoter** | 5 分钟 | 扫描工作记忆，把高 importance 条目晋升到情景记忆 |
| **语义 consolidator** | 10 分钟 | 聚合情景事件，更新用户画像和群体画像 |
...
- `working_memory.json` — 各群聊的工作记忆窗口
- `assistant_emotion.json` — 助手自身情感状态
- `group_sentiment.json` — 群体情感缓存
- `token_usage.json` — token 用量统计
- `persona.json` — 人格设定

## 与 Legacy 引擎的对比

| 维度 | EmotionalGroupChatEngine (v0.28) | AsyncRolePlayEngine (legacy) |
|------|-----------------------------------|------------------------------|
| 回复决策 | 动态阈值 + 四层策略 | 静态概率 + 二元 should_reply |
| 情绪 | 2D valence-arousal + 助手自身情绪 | 无 |
| 记忆 | 三层（工作/情景/语义） | 单层用户事实 + 事件记忆 v2 |
| 延迟回复 | 有（话题间隙触发） | 无 |
| 主动发言 | 有（时间/记忆/情感触发） | 无 |
| 群聊隔离 | 是 | 否（v0.28 迁移后才隔离） |
| 模型路由 | 按任务类型自动选择 | 固定模型 |
...
# reply.reply 为 None 表示决定不回复（SILENT / DELAYED / PROACTIVE）
```
