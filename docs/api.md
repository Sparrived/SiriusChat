...
> **v0.28+ 重要提示**：本文档包含 Legacy API �?Emotional Engine API�?> 新用户应优先使用 `create_emotional_engine()` �?`EmotionalGroupChatEngine`�?> Legacy `AsyncRolePlayEngine` 已归档，不再接收新功能�?
## 模块索引

- [engine](#engine)
- [memory](#memory)
- [models](#models)
- [orchestration](#orchestration)（Legacy�?- [prompting](#prompting)
- [providers](#providers)
- [session](#session)
...
**参数�?*
- `work_path`: workspace 路径（持久化目录�?- `provider`: LLM provider（可选）
...
**返回�?* `dict[str, Any]` 包含�?- `strategy`: `"immediate"` / `"delayed"` / `"silent"` / `"proactive"`
- `reply`: 说出口的话（来自 `<say>`�?- `thought`: 内心独白（来�?`<think>`�?- `emotion`: 情绪状态字�?- `intent`: 意图分析字典
...
多模型协同必需启用，支持两种配置方案：
1. unified_model: 所有任务使用同一个模�?2. task_models: 为每个任务独立配置模�?
...
- `to_orchestration_policy(self) -> OrchestrationPolicy` - 转换�?OrchestrationPolicy 对象�?
### Functions
...
在现有会话配置中设置多模型编排�?
Args:
    session_config: 现有�?SessionConfig 对象
    task_models: 任务模型映射，例�?{"memory_extract": "model-1", "event_extract": "model-2", "intent_analysis": "model-3"}
    task_temperatures: 各任务的采样温度，例�?{"memory_extract": 0.1}
    task_max_tokens: 各任务的最�?token 数，例如 {"memory_extract": 128}
    task_retries: 各任务的重试次数，例�?{"memory_extract": 1}
    max_multimodal_inputs_per_turn: 每轮最多多模态输入数（默�?4�?    max_multimodal_value_length: 多模态值最大长度（默认 4096�?
Returns:
    配置完成�?SessionConfig 对象（原对象已修改）
...
创建多模型配置对象�?
返回 MultiModelConfig 对象，可以用于后续设置或转换�?OrchestrationPolicy�?
Args:
    task_models: 任务模型映射
    task_temperatures: 任务采样温度
    task_max_tokens: 任务最�?token �?    task_retries: 任务重试次数
    max_multimodal_inputs_per_turn: 最多多模态输入数
    max_multimodal_value_length: 多模态值最大长�?
Returns:
    MultiModelConfig 对象
