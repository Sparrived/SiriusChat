...
12. `sirius_chat/core/response_assembler.py`
...
- Legacy `AsyncRolePlayEngine` 已归档到 `sirius_chat/core/_legacy/`，不再接收新功能。
...
- 用户记忆已改为群隔离：`UserMemoryManager.entries` 为 `{group_id: {user_id: UserMemoryEntry}}`。
- 语义记忆通过 `_consolidate_group()` 周期性更新，优先从 `event_memory.entries` 读取结构化观察。
