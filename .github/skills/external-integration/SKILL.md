...
- `reply_mode=auto` 的意愿系统参数可在 `OrchestrationPolicy` 调整：用户节奏阈值（`auto_reply_user_cadence_seconds`）、群聊窗口（`auto_reply_group_window_seconds`）、群聊惩罚起点（`auto_reply_group_penalty_start_count`）、AI 冷却（`auto_reply_assistant_cooldown_seconds`）、阈值（`auto_reply_threshold`）与阈值上调起点（`auto_reply_threshold_boost_start_count`）。
...
- ✨ **(v0.13.0)** 回复频率限制：`reply_frequency_window_seconds`（默认60s）内超过 `reply_frequency_max_replies`（默认8）次时跳过回复，提及 AI 名字时免除（`reply_frequency_exempt_on_mention=True`）。
- ✨ **意图分析** (v0.10.0)：通过 `OrchestrationPolicy.enable_intent_analysis=True` 启用 LLM 意图分析，自动调整回复意愿分数和优化系统提示词 token 消耗。默认关闭，使用零开销关键词回退路径。
