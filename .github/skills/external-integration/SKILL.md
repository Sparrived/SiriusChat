...
5. `sirius_chat/config/manager.py` ✨ (P1-006 配置管理)
...
- 若调用方不希望手动维护用户与持久化，优先使用 `WorkspaceRuntime` / `open_workspace_runtime(...)`；外部至少传 `work_path`、`session_id` 和业务输入，必要时再传独立 `config_path`，runtime 自动处理恢复、落盘与基于文件监听的配置热刷新。
