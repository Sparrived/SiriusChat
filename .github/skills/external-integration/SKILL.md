...
- 若调用方不希望手动维护用户与持久化，优先使用 `WorkspaceRuntime` / `open_workspace_runtime(...)`；外部至少传 `work_path`、`session_id` 和业务输入，必要时再传独立 `config_path`，runtime 自动处理恢复、落盘与基于文件监听的配置热刷新。可通过 `bootstrap` 参数在首次打开时注入 agent key、session defaults 和 provider entries。
