...
- 若调用方不希望手动维护用户与持久化，优先使用 `WorkspaceRuntime` / `open_workspace_runtime(...)`；外部只传 `work_path`、`session_id` 和业务输入，runtime 自动处理恢复与落盘。
- 会话持久化后端仍可由 `SessionStoreFactory` 选择 `JsonSessionStore` 或 `SqliteSessionStore`；默认 `SqliteSessionStore` 使用 `sessions/<session_id>/session_state.db`，并会在首次打开时自动迁移 legacy `session_state.json` 或旧的 payload-style SQLite。
...
- 交互模式下可用 `/provider platforms|add|remove|list` 管理 API Key（持久化在 `work_path/providers/provider_keys.json`）。
