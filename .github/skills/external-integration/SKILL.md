...
- ✨ **SKILL 系统**：通过 `skills/` 模块让 AI 在运行时调用外部 Python 代码
  - 启用：`OrchestrationPolicy(enable_skills=True, skill_execution_timeout=30.0)`，SKILL 文件放在 `{work_path}/skills/` 目录下
  - SKILL 文件需导出 `SKILL_META` 字典（含 name, description, parameters）和 `run(**kwargs)` 函数
  ...
  - 导入：`from sirius_chat import SkillRegistry, SkillExecutor, SkillDataStore`
