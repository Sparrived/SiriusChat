...
- ✨ **参与决策系统** (v0.14.0)：三级架构替代旧意愿分系统：HeatAnalyzer（零 LLM 开销热度分析）→ IntentAnalyzer v2（意图分类 + target 识别）→ EngagementCoordinator（融合决策）。LLM 意图分析现由 `intent_analysis` 任务驱动，可通过 `task_enabled/task_models/task_temperatures/task_max_tokens/task_retries` 精细控制；任务关闭时使用关键词回退，但任务启用后若调用失败或解析失败，不再自动降级为关键词意图推断。多 AI 群聊里，分析器会进一步区分“当前模型自身”与“其他 AI”，并在后者场景下抑制当前模型自动回复；为降低误判，传给模型的上下文已改为最近交互链摘要，并会显式附带最近 AI / 人类发言者、近期发言人的 aliases、`environment_context` 环境线索，以及当前消息命中的当前模型/其他 AI/人类名字线索。对未明确点名当前模型的群控/停用类命令，还会做硬抑制，不触发当前模型回复。
...
- ✨ **SKILL 系统**：通过 `skills/` 模块让 AI 在运行时调用外部 Python 代码
  ...
  - 引擎自动检测 AI 回复中的内置 `[SKILL_CALL: name | {params}]` 标记并执行，结果注入上下文后重新生成
