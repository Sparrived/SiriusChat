...
- 若需启用提示词驱动的内容分割，配置 `OrchestrationPolicy.enable_prompt_driven_splitting=True` 即可；分割标记为框架内置的 `<MSG_SPLIT>`。
...
- **会话事件流** (v0.9.0)：`on_message` 回调已移除，改用 `engine.subscribe(transcript)` 实时事件流。外部系统应在 `run_live_session` 后启动后台任务订阅 `SessionEvent`，实时接收 AI 回复、SKILL 状态等事件。`SKILL_COMPLETED` 只提供状态字段，不携带技能结果正文；真正要发到外部平台的内容应只消费 assistant 消息。详见 `docs/migration-event-stream.md`。
...
- `OrchestrationPolicy.min_reply_interval_seconds` 可用于控制两次 AI 实际回复之间的最小间隔；间隔内的新消息会继续留在 `WorkspaceRuntime` 队列中，等窗口结束后先按静默批处理方式合并，再进入正常回复判断。
...
- `WorkspaceRuntime.run_live_message(...)` 采用单会话队列语义：消息先入队；当待处理数量超过 `pending_message_threshold` 时会静默合并同一说话人的连续消息；若配置了 `min_reply_interval_seconds`，则会在 AI 刚回复后的冷却窗口内继续蓄积消息，并在窗口结束后先做一次同样的合并再送进 engine。
...
- ✨ **后台记忆归纳** (v0.10.0)：引擎自动静默启动后台循环，定时使用 `memory_manager` 任务模型归纳合并冗余事件/摘要/事实；当 finalize 时发现上下文已很长，也会立即补跑一轮。通过 `consolidation_interval_seconds` 设置间隔；若不希望继续调用该模型，关闭 `task_enabled["memory_manager"]`。
...
- ✨ **SKILL 系统**：通过 `skills/` 模块让 AI 在运行时调用外部 Python 代码
  - 默认：`enable_skills=True`；框架会先加载包内置 SKILL（当前包含 `system_info` 与 developer-only 的 `desktop_screenshot`），再加载 workspace `skills/` 目录。SKILL 文件默认放在 `{work_path}/skills/`，双根布局时位于 `config_root/skills/`。若只想保留目录结构、不执行 SKILL，可显式设置 `enable_skills=False`
  ...
  - SKILL 文件需导出 `SKILL_META` 字典（含 name, description, parameters, 可选 dependencies 与 `developer_only`）和 `run(**kwargs)` 函数；如需审计调用者，可显式接收 `invocation_context`
