...
- ✨ **(v0.14.5)** AI 自身记忆触发机制改为**定时后台任务**：`OrchestrationPolicy.self_memory_extract_interval_seconds`（默认 360 秒），由 `BackgroundTaskManager._self_memory_loop` 周期运行；`self_memory_extract_batch_size` 保留但不再触发提取。新增 `max_concurrent_llm_calls`（默认 1）限制单 session LLM 并发数，算法路径不受影响，`0` 表示不限制。
