...
3. 命令校验
   ...
   - 使用独立工作目录运行 `python main.py --config examples/session.json --work-path data/release_smoke_v023`，验证首次引导和入口可用
   - 首次仅进入并退出时，检查 `data/release_smoke_v023/workspace.json` 与 `data/release_smoke_v023/sessions/default/participants.json` 是否成功写出
   - 若要验证 `data/release_smoke_v023/sessions/default/session_state.db`，需在可用 provider 下完成至少一轮真实消息往返
4. 文档与 AI 资产同步
