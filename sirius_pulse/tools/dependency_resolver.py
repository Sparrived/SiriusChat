"""Resolve and install missing dependencies for TOOL files.

Strategy:
1. Parse TOOL_META.dependencies (explicit list, preferred).
2. Fallback: scan top-level import statements in the TOOL source.
...
def resolve_tool_dependencies(
    ...
    *,
    ...
    """Check a TOOL file for missing dependencies and optionally install them.

    Returns the list of packages that were successfully installed (may be empty).
    """
    declared = _extract_declared_dependencies(tool_file)
    imported = _extract_imported_packages(tool_file)
    ...
    if not auto_install:
        logger.warning(
            "TOOL '%s' 存在未安装的依赖: %s（auto_install 已关闭）",
            tool_file.name,
            ", ".join(sorted(missing)),
        )
        return []

    return _install_packages(sorted(missing), tool_file.name)
...
def _extract_declared_dependencies(tool_file: Path) -> set[str]:
    """Read ``TOOL_META["dependencies"]`` without executing the module."""
    try:
        source = tool_file.read_text(encoding="utf-8")
        tree = ast.parse(source, filename=str(tool_file))
    except (SyntaxError, OSError) as exc:
        logger.debug("AST解析TOOL文件失败 (%s): %s", tool_file.name, exc)
        return set()
...
def _extract_imported_packages(tool_file: Path) -> set[str]:
...
def _install_packages(packages: list[str], tool_name: str) -> list[str]:
    ...
    try:
        ...
        logger.error("TOOL '%s': 依赖安装超时", tool_name)
        return []
    except FileNotFoundError:
        logger.error("TOOL '%s': 找不到包管理器 (%s)", tool_name, installer)
        return []
