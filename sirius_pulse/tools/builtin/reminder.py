"""Built-in tool for creating timed reminders.

Supports both active (model-callable) and passive (background task) modes.
- Active: AI invokes the reminder tool via function_call to create/list/cancel.
...
_config.group("高级设置").add(
    ...
    description=(
        "触发提醒时预先执行的工具调用链。"
        '每项格式：{"tool": "工具名", "params": {"参数": "值"}}。'
        "执行结果会作为上下文输入给模型，供生成提醒消息时参考。"
    ),
    fields=[
        {"name": "tool", "type": "str", "description": "工具名称", "required": True},
    ...
    for r in reminders:
        if _is_reminder_due(r, now):
            ...
            if gid:
                ...
                tool_chain = r.get("tool_chain")
                tool_results: list[dict[str, Any]] = []
                if tool_chain:
                    tool_results = await _execute_tool_chain(
...
async def _execute_tool_chain(
    ...
    """Execute a tool chain defined in a reminder."""
    ...
    logger.info("Reminder %s tool_chain start: %d items", reminder_id, len(tool_chain))
    tool_results: list[dict[str, Any]] = []
    for item in tool_chain:
        ...
        try:
            ...
            if tool is None:
                logger.warning("Reminder %s tool '%s' not found", reminder_id, tool_name)
                tool_results.append({"tool": tool_name, "params": params, "error": "未找到"})
                continue
            result = await executor.execute_async(tool, params, invocation_context=inv_ctx)
            tool_results.append(
                {
                    "tool": tool_name,
                    "params": params,
            ...
            logger.warning("Reminder %s tool_chain failed: %s -> %s", reminder_id, tool_name, exc)
            tool_results.append({"tool": tool_name, "params": params, "error": str(exc)})

    logger.info(
        "Reminder %s tool_chain finished: %d/%d succeeded",
        reminder_id,
        sum(1 for sr in tool_results if "error" not in sr),
        len(tool_results),
    )
    return tool_results
...
async def _generate_reminder_message(
    ...
    try:
        ...
        tool_desc = ctx.get_tool_descriptions(caller_is_developer=False)
...
def _do_create(
    ...
    if tool_chain:
        reminder["tool_chain"] = tool_chain
