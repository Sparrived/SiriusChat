...
export async function init(container, params = {}) {
  ...
  container.innerHTML = `
    <div class="card" style="margin-bottom:20px">
      <div class="card-header">
        <div>
          <div class="card-title">Tool 调用追踪</div>
          <div class="card-subtitle">查看 ${name} 的工具执行历史</div>
        </div>
        <div style="display:flex;gap:12px;align-items:center">
          <select id="toolFilter" class="btn btn-sm">
  ...
  const toolFilterEl = $('toolFilter');
  if (toolFilterEl) {
    toolFilterEl.addEventListener('change', (e) => {
...
async function loadHistory(silent = false) {
  ...
  try {
    const data = await get(`/persona/tool-history?${params}`);
    ...
    if (silent) {
      console.warn('tool history realtime refresh failed:', e);
      return;
    }
...
function updateToolFilter() {
  const pageTools = historyData.map(h => h.tool_name);
  for (const s of pageTools) {
    if (!knownTools.includes(s)) knownTools.push(s);
  }
  knownTools.sort();

  const sel = $('toolFilter');
  if (!sel) return;
  const current = sel.value;
  sel.innerHTML = `<option value="">全部工具</option>` +
    knownTools.map(s => `<option value="${s}"${s === current ? ' selected' : ''}>${s}</option>`).join('');
}
          ...
          <div style="padding:16px;background:var(--bg-1);border-radius:8px;border-left:3px solid ${statusColor}">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
              <div style="display:flex;align-items:center;gap:8px">
                ...
                <span style="font-weight:600;font-size:14px">${h.tool_name}</span>
